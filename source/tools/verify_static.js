...
(async () => {
  ...
  /* ---- ⑤ vendor 라이브러리 경고 -------------------------------------- */
  /* three.js UMD 빌드(build/three.min.js)는 r150+ 에서 deprecated 라 **파일 스스로**
     콘솔에 경고를 찍는다. ESM 빌드를 IIFE 로 감싸 쓰도록 바꿨고(tools/fetch_three.py),
     되돌아가지 않는지 여기서 지킨다. (2026-08-19) */
  const dep = errs.concat(warns).filter(m => /deprecated/i.test(m));
  ok('⑤ vendor 라이브러리 deprecated 경고 없음', dep.length === 0, dep.slice(0, 2).join(' | ') || '경고 0건');
  const three = await p.evaluate(() => ({
    rev: window.THREE && window.THREE.REVISION,
    keys: window.THREE ? Object.keys(window.THREE).length : 0,
    /* ESM 빌드의 최상위 한 글자 선언(const t·e·n …)이 전역으로 새면
       뒤따르는 스크립트의 선언과 충돌한다 — IIFE 로 감싼 이유다 */
    leak: ['t', 'e', 'n', 'i', 's'].filter(k => k in window),
  }));
  ok('⑤ three.js 가 전역 THREE 로 올라온다', three.rev && three.keys > 300, `r${three.rev} · ${three.keys}개`);
  ok('⑤ three.js 최상위 이름이 전역으로 새지 않는다', three.leak.length === 0, three.leak.join(', ') || '누수 0건');
